...
```bash
npx tsc -p tsconfig.node.json && node dist-electron/server/start.js
```

Esto compila el backend y lo ejecuta en el puerto 3847. Deberias ver:

```
El Cofre API running on http://127.0.0.1:3847
El Cofre API ready on http://127.0.0.1:3847
```
...
| Que | Comando |
|-----|---------|
| Correr backend | `npx tsc -p tsconfig.node.json && node dist-electron/server/start.js` |
